...
def ping_server(host, port=25565, timeout=3.0) -> Dict[str,Any]:
    hr, pr = resolve_srv(host, port)
    with socket.create_connection((hr, pr), timeout=timeout) as s:
        s.settimeout(timeout)
        data = varint(765) + varint(len(hr)) + hr.encode() + struct.pack(">H", pr) + varint(1)
        ...
        return {"host": host, "resolved": hr, "port": pr, "version": version, "online": online, "max": maxp, "motd": motd}
    ...
    with p.open("r", encoding="utf-8", errors="ignore") as f:
        for raw in f:
            ...
            if line.lower().startswith("open ") or " open " in (" " + line.lower() + " "):
                parts = line.split()
                # try classic masscan: open tcp 1.2.3.4 25565
                try:
